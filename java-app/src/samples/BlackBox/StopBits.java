...
class StopBits extends Choice implements ItemListener
{
	...
	public void itemStateChanged( ItemEvent ev )
	{
		...
		if ((value > 0) && (port != null))
		{
			 ...
			 * 
			 * Note: we must set all of the parameters, not just the number of stop bits, hence the use of get*
			 */

			try
			{
				port.setSerialPortParams(port.getBaudRate(), port.getDataBits(), value, port.getParity());
			}

			catch (UnsupportedCommOperationException e)
			{
				System.out.println("Cannot set stop bit size to " + sel + " for port " + port.getName());
			}
		}
