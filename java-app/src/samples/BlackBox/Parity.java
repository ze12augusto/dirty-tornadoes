...
class Parity extends Choice implements ItemListener
{
	...
	public void itemStateChanged( ItemEvent ev )
	{
		...
		if ((value > 0) && (port != null))
		{
			 ...
			 * Set the parity.
			 * 
			 * Note: we must set all of the parameters, not just the parity, hence the use of get*
			 */

			try
			{
				port.setSerialPortParams(port.getBaudRate(), port.getDataBits(), port.getStopBits(), value);
			}

			catch (UnsupportedCommOperationException e)
			{
				System.out.println("Cannot set parity to " + sel + " for port " + port.getName());
			}
		}
