...
public class ByteStatistics extends Panel
{
	...
	public ByteStatistics( String text, int size, SerialPort port, boolean inputBuffer, boolean average )
	{
		super();
	...
	public ByteStatistics( String text, int size, SerialPort port, boolean inputBuffer )
	{
	...
	{
		/*
		 * if (port != null) { if (inputBuffer) { System.out.println("Reseting receive baud rate for " + port.getName()); }
		 * 
		 * else { System.out.println("Reseting transmit baud rate for " + port.getName()); } }
		 */
		this.lastTime = 0;
		this.lastBaudRate = 0;
	}
	...
	{
		long baudRate, time = System.currentTimeMillis(), val = this.counter.getValue();
		...
		{
			baudRate = ((val - this.lastCount) * this.bitsPerCharacter * 1000) / (time - this.lastTime);
			...
			{
				/*
				 * This is a silly hack to auto-reset the counter if the user stops transmitting
				 */
