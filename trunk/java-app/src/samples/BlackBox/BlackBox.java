...
public class BlackBox extends Frame implements WindowListener
{
	...
	static BlackBox win;
	static boolean threaded = true, silentReceive = false, modemMode = false, friendly = false;
		...
		while (args.length > idx)
		{
			...
			else if (args[idx].equals("-p"))
			{
				...
				while (args.length > idx)
				{
					...
					try
					{
						portId = CommPortIdentifier.getPortIdentifier(args[idx]);
	...
	private void addPort( CommPortIdentifier portId )
	{
		...
		if (portId.getPortType() == CommPortIdentifier.PORT_SERIAL)
		{
			...
			try
			{
				portDisp[portNum] = new SerialPortDisplay(portId, threaded, friendly, silentReceive, modemMode, rcvDelay, win);
	...
	private void cleanup()
	{
		...
		while (portNum > 0)
		{
			...
			if (p != null)
			{
				System.out.println("Closing port " + portNum + " (" + p.getName() + ")");
