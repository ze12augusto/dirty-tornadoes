...
class BaudRate extends Choice implements ItemListener
{
	...
	public void itemStateChanged( ItemEvent ev )
	{
		...
		if ((value > 0) && (port != null))
		{
			 ...
			 * Set the baud rate.
			 * 
			 * Note: we must set all of the parameters, not just the baud rate, hence the use of get*
			 */

			try
			{
				port.setSerialPortParams(value, port.getDataBits(), port.getStopBits(), port.getParity());
			}

			catch (UnsupportedCommOperationException e)
			{
				System.out.println("Cannot set baud rate to " + sel + " for port " + port.getName());
			}
		}
