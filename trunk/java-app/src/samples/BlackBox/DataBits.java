...
class DataBits extends Choice implements ItemListener
{
	...
	public void itemStateChanged( ItemEvent ev )
	{
		...
		if ((value > 0) && (port != null))
		{
			 ...
			 * 
			 * Note: we must set all of the parameters, not just the number of data bits, hence the use of get*
			 */

			try
			{
				port.setSerialPortParams(port.getBaudRate(), value, port.getStopBits(), port.getParity());
			}

			catch (UnsupportedCommOperationException e)
			{
				System.out.println("Cannot set data bit size to " + sel + " for port " + port.getName());
			}
		}
